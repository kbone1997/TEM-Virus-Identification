...
def get_models():
    ...
    conv1 = Conv2D(64, (3, 3), activation='relu', padding='same', kernel_initializer=initializers.glorot_uniform(W_SEED[1]), kernel_regularizer=reg_kernel, name='conv1_1')(inputs)
    conv1 = BatchNormalization()(conv1)
    conv1 = Conv2D(63, (3, 3), activation='relu', padding='same', kernel_initializer=initializers.glorot_uniform(W_SEED[2]), kernel_regularizer=reg_kernel, name='conv1_2')(conv1)
    conv1 = BatchNormalization()(conv1)
    conv1 = concatenate([conv1, inputs], axis=AXIS)
    pool1 = MaxPooling2D(pool_size=(2, 2))(conv1)
    pool1 = Dropout(rate=0.2)(pool1)
    
    conv2 = Conv2D(64, (3, 3), activation='relu', padding='same', kernel_initializer=initializers.glorot_uniform(W_SEED[3]), kernel_regularizer=reg_kernel, name='conv2_1')(pool1)
    conv2 = BatchNormalization()(conv2)
    conv2 = Conv2D(64, (3, 3), activation='relu', padding='same', kernel_initializer=initializers.glorot_uniform(W_SEED[4]), kernel_regularizer=reg_kernel, name='conv2_2')(conv2)
    conv2 = BatchNormalization()(conv2)
    conv2 = concatenate([conv2, pool1], axis=AXIS)
    pool2 = MaxPooling2D(pool_size=(2, 2))(conv2)
    pool2 = Dropout(rate=0.25)(pool2)
    
    conv3 = Conv2D(128, (3, 3), activation='relu', padding='same', kernel_initializer=initializers.glorot_uniform(W_SEED[5]), kernel_regularizer=reg_kernel, name='conv3_1')(pool2)
    conv3 = BatchNormalization()(conv3)
    conv3 = Conv2D(128, (3, 3), activation='relu', padding='same', kernel_initializer=initializers.glorot_uniform(W_SEED[6]), kernel_regularizer=reg_kernel, name='conv3_2')(conv3)
    conv3 = BatchNormalization()(conv3)
    conv3 = concatenate([conv3, pool2], axis=AXIS)
    pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)
    pool3 = Dropout(rate=0.3)(pool3)

    conv4 = Conv2D(128, (3, 3), activation='relu', padding='same', kernel_initializer=initializers.glorot_uniform(W_SEED[7]), kernel_regularizer=reg_kernel, name='conv4_1')(pool3)
    conv4 = BatchNormalization()(conv4)
    conv4 = Conv2D(128, (3, 3), activation='relu', padding='same', kernel_initializer=initializers.glorot_uniform(W_SEED[8]), kernel_regularizer=reg_kernel, name='conv4_2')(conv4)
    conv4 = BatchNormalization()(conv4)
    conv4 = concatenate([conv4, pool3], axis=AXIS)
    pool4 = MaxPooling2D(pool_size=(2, 2))(conv4)
    pool4 = Dropout(rate=0.35)(pool4)

    # conv5 = Conv2D(128, (3, 3), activation='relu', padding='same', kernel_initializer=initializers.glorot_uniform(W_SEED[9]), name='conv5_1')(pool4)
    # conv5 = BatchNormalization()(conv5)
    # conv5 = Conv2D(128, (3, 3), activation='relu', padding='same', kernel_initializer=initializers.glorot_uniform(W_SEED[10]), name='conv5_2')(conv5)
    # conv5 = BatchNormalization()(conv5)
    # conv5 = concatenate([conv5, pool4], axis=AXIS)
    # pool5 = MaxPooling2D(pool_size=(2, 2))(conv5)
    # pool5 = Dropout(rate=0.35)(pool5)

    # conv6 = Conv2D(256, (3, 3), activation='selu', padding='same', kernel_initializer=initializers.glorot_uniform(W_SEED[11]), name='conv6_1')(pool5)
    # conv6 = BatchNormalization()(conv6)
    # conv6 = Conv2D(256, (3, 3), activation='selu', padding='same', kernel_initializer=initializers.glorot_uniform(W_SEED[12]), name='conv6_2')(conv6)
    # conv6 = BatchNormalization()(conv6)
    # conv6 = concatenate([conv6, pool5], axis=AXIS)
    # pool6 = MaxPooling2D(pool_size=(2, 2))(conv6)
    # pool6 = Dropout(rate=0.25)(pool6)

    pool6 = Conv2D(256, (3, 3), activation='relu', padding='valid', kernel_initializer=initializers.glorot_uniform(W_SEED[21]), kernel_regularizer=reg_kernel, name='conv_LAST')(pool4)
    flat = Flatten()(pool6)
    
    dense = Dense(256, activation='relu', kernel_initializer=initializers.glorot_uniform(W_SEED[31]), kernel_regularizer=reg_kernel)(flat)
    dense = Dropout(rate=0.35)(dense)  
    dense = Dense(256, activation='relu', kernel_initializer=initializers.glorot_uniform(W_SEED[32]), kernel_regularizer=reg_kernel)(dense)   
    output = Dense(CLASSES_NO, activation=None, kernel_initializer=initializers.glorot_uniform(W_SEED[33]))(dense)
...
def train_CNN(x, y, x_v, y_v, modelName):
    ...
    # callbacks
    modelCheckpoint = ModelCheckpoint(modelDir+'/'+modelName+'_weights_ACCURACY_BEST.h5', monitor='val_categorical_accuracy', save_best_only=True)
    modelCheckpointLoss = ModelCheckpoint(modelDir+'/'+modelName+'_weights_LOSS_BEST.h5', monitor='val_loss', save_best_only=True)    
